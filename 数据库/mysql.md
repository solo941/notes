...
- 索引无法存储null值
- 如果条件中有OR值,即使其中有条件带索引也不会使用(这也是为什么尽量少用OR的原因),除非给OR条件的每个列都加上索引
- 对于多列索引,不是使用的第一部分,就不会使用索引
- like以%开头不会使用索引
- 不以引号引用的字符串不会使用索引
- 如果全表更快不会使用索引
- 联合索引必须加入左前缀(第一个),否则不会使用索引,比如联合索引id+name,只查询name就不会使用索引,
  因索引底层采用b+树叶子节点顺序排列,必须通过左前缀索引才能定位到具体的节点范围

## 创建表时的注意事项

1.不使用enum类型

原因：enum类型order by效率低；枚举值是数值时，会有陷阱

test(foobar ENUM('0','1','2'));
...
以分为单位，可以用Int类型，如果用元，用decimal类型。

3.时间用什么类型

varchar:优点直观，缺点没有校验，比如2013111有二义性

timestamp:1970-2038年，4个字节，优点带时区信息，随着数据库的时区改变，适合跨时区的应用

datetime与bigint，占用8个字节，需要自己变更维护时间轴。

4.为什么不用数据库存储图片，音频等大文件

不能使用内存临时表，binlog内容太多

5.字段为什么要定义NOT NULL

索引性能不好，空列被索引后，每条记录需要一个额外的字节；
